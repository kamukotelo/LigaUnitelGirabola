          ...
          <AnimatedCard variant="hud" className="bg-zinc-100/40 dark:bg-zinc-950/40 border-zinc-200 dark:border-zinc-900 p-6">
            ...
            <div className="space-y-4">
              {matches.slice(0, 3).map((match) => {
                ...
                return (
                  <Link key={match.id} href={`/matches/${match.id}`} className="p-3 bg-white/30 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-900 hover:border-accent/30 transition-colors rounded-xl flex items-center justify-between gap-4 font-mono text-xs">
                    <div>
                      <span className="text-[8px] text-zinc-500 block uppercase">JORNADA {match.round}</span>
                      <span className="text-foreground font-bold block truncate max-w-[120px]">{opponent}</span>
                      <span className="text-[9px] text-zinc-500">{isHome ? 'Casa' : 'Fora'}</span>
                    </div>

                    {isFinished ? (
                      <div className="px-2.5 py-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded text-center">
                        <span className="text-foreground font-bold block">{match.score}</span>
