...
import type { MatchRecord, MatchRecordLineupPlayer, SeasonDerivedStats } from '../data/jogos/tipos';
...
export interface Team {
  ...
  dataStatus?: string;      // estado editorial recebido na base oficial
  dataUpdatedAt?: string;   // data da última atualização da base (yyyy-mm-dd)
}
...
export interface Match {
  ...
  attendance?: number;   // assistência oficial
  usefulTimeMinutes?: number; // tempo efetivo apenas quando publicado oficialmente
}

// Registo único por jogo (src/data/jogos/2026-27/<id>.ts): agenda, resultado,
// arbitragem, escalações, eventos e estatísticas vivem num só ficheiro. Tudo o
// que o portal mostra sobre um jogo — calendário, início, hub, API, ficha,
// classificação e estatísticas — sai destes registos.
const MATCH_RECORD_BY_ID: ReadonlyMap<string, MatchRecord> = new Map(
  MATCH_RECORDS_2026_27.map((record) => [record.id, record]),
);
// A agenda aplica-se por confronto (jornada + equipas), para continuar a servir
// um calendário persistido na BD cujos IDs possam diferir dos do sorteio.
const MATCH_RECORD_BY_FIXTURE: ReadonlyMap<string, MatchRecord> = new Map(
  MATCH_RECORDS_2026_27.map((record) => [`${record.round}:${record.homeTeamId}:${record.awayTeamId}`, record]),
);

/** Registo publicado de um jogo da época em curso, quando existe. */
export function getMatchRecord(matchId: string): MatchRecord | undefined {
  return MATCH_RECORD_BY_ID.get(matchId);
}

// Data editorial mínima das etiquetas "atualizado em": cada resultado traz a
...
export function applyOfficialMatchSchedule(matches: Match[]): Match[] {
  return sortOfficialMatches(matches.map((match) => {
    const record = MATCH_RECORD_BY_FIXTURE.get(`${match.round}:${match.homeTeamId}:${match.awayTeamId}`);
    if (!record) {
    ...
    // O registo substitui por completo agenda e resultado: um campo ausente no
    // ficheiro (ex.: `broadcaster`) nunca é herdado do sorteio importado.
    const { schedule, result } = record;
    return normalizeMatchOverride({
      ...match,
      date: schedule.date,
      ...
      liveMinute: result?.liveMinute,
      attendance: result?.attendance,
      usefulTimeMinutes: result?.usefulTimeMinutes,
...
/**
 * Goleadores da época em curso, calculados a partir dos golos publicados nos
 * registos de jogo (ver computeSeasonDerivedStats). Vêm de um ficheiro gerado
 * porque o plantel usa estes totais e a conciliação das fichas usa o plantel.
 */
export const CURRENT_SEASON_SCORERS = SEASON_DERIVED_STATS_2026_27.scorers;

/** Cartões por jogador, calculados a partir dos eventos dos registos de jogo. */
const CURRENT_CONFIRMED_CARDS: Readonly<Record<string, { yellow: number; red: number }>> = SEASON_DERIVED_STATS_2026_27.cards;
...
export interface Player extends PlayerStats {
  ...
  nationality: string;
  height: string;
  statsVerified?: boolean;   // métricas avançadas só aparecem quando confirmadas
  weight?: string;
  // ── Ficha de identidade ──
...
export const TEAMS: Team[] = [
  ...
  { id: 'caala', name: 'CR Caála', officialName: 'Clube Recreativo da Caála', shortName: 'CRC', logoUrl: TEAM_CRESTS['caala'], city: 'Huambo', stadium: 'Estádio Daniel Cassoma Lutucuta', stadiumCapacity: 12000, founded: 1944, colors: 'Azul e Branco', coach: 'Artur Benjamim Correia', president: 'Mi Mosquito', nickname: 'Caalenses', colorsHex: ['#00529B', '#FFFFFF'], kits: [{ label: 'Principal', colors: ['#00529B', '#FFFFFF'] }, { label: 'Secundário', colors: ['#FFFFFF', '#00529B'] }], dataStatus: 'Atualizado', dataUpdatedAt: '2026-08-31' },
...
function enrichPlayer(p: Player): Player {
  ...
  if (CURRENT_SEASON_SCORERS.some((scorer) => scorer.id === p.id) || Object.values(p.attributes).every((value) => value === 0)) {
    return {
      ...p,
      technicalRating: 0,
      formRating: 0,
...
// Plantel confirmado do 1.º de Agosto, consolidado a partir das
// convocatórias e ocorrências oficiais já publicadas no portal.
const DAGO_SQUAD_2026_27: Player[] = [
  ['nuno-dago', 'Nuno', 'Guarda-redes', 1],
  ['anselmo-dago', 'Anselmo', 'Guarda-redes', 22],
  ['milton-dago', 'Milton', 'Defesa', 2],
  ['mabele-dago', 'Mabelé', 'Defesa', 3],
  ['bonifacio-dago', 'Bonifácio', 'Defesa', 5],
  ['macaia-dago', 'Macaia', 'Defesa', 16],
  ['bulaya-dago', 'Bulaya', 'Defesa', 28],
  ['bencao-dago', 'Benção', 'Defesa', 36],
  ['mabilson-dago', 'Mabilson', 'Médio', 7],
  ['calebi-dago', 'Calebi', 'Médio', 10],
  ['venancio-dago', 'Venâncio', 'Médio', 15],
  ['cliver-dago', 'Clíver', 'Médio', 18],
  ['tombe-dago', 'Tombé', 'Médio', 20],
  ['castro-dago', 'Castro', 'Médio', 27],
  ['bruno-dago', 'Bruno', 'Avançado', 6],
  ['axel-dago', 'Axel', 'Avançado', 8],
  ['rupson-dago', 'Rupson', 'Avançado', 9],
  ['fernando-dago', 'Fernando', 'Avançado', 11],
  ['obed-dago', 'Obed', 'Avançado', 14],
  ['dago-tshibamba', 'Dagó Tshibamba', 'Avançado', 17],
].map(([id, name, position, jerseyNumber]) => ({
  id: String(id), name: String(name), club: '1.º de Agosto', teamId: 'dago', position: String(position),
  goals: 0, assists: 0, appearances: 0, jerseyNumber: Number(jerseyNumber), age: 0,
...
// Plantel confirmado do Desportivo da Huíla. Os nomes extensos recebidos nas
// ocorrências substituem as alcunhas curtas da convocatória quando o número
// de camisola permite a correspondência inequívoca.
const HUILA_SQUAD_2026_27: Player[] = [
  ['ndulo-huila', 'Ndulo', 'Guarda-redes', 1],
  ['passy-huila', 'Passy', 'Guarda-redes', 12],
  ['katendi-huila', 'Katendi', 'Defesa', 2],
  ['dos-santos-huila', 'Dos Santos', 'Defesa', 4],
  ['joao-milagre-huila', 'João Milagre Chiva Simões', 'Defesa', 6],
  ['leonardo-isola-huila', 'Leonardo Manuel Isola Ramos (Cabibi)', 'Defesa', 7],
  ['lucas-elias-huila', 'Lucas Elias Antonio Paulo (Ludy)', 'Defesa', 13],
  ['jeremias-huila', 'Jeremias', 'Defesa', 26],
  ['mauricio-pedro-huila', 'Mauricio Pedro (Jeizi)', 'Médio', 8],
  ['mendes-huila', 'Mendes', 'Avançado', 10],
  ['elias-huila', 'Elias', 'Médio', 15],
  ['milton-suca-huila', 'Milton Alberto de Oliveira Suca', 'Avançado', 18],
  ['toyzinho-huila', 'Toyzinho', 'Médio', 20],
  ['angelo-cangu-huila', 'Angelo Cangu (Tchutchu)', 'Médio', 21],
  ['milagre-simba-huila', 'Milagre Carlos Simba', 'Avançado', 25],
  ['antonio-huila', 'António', 'Médio', 27],
  ['pequenino-castro-huila', 'Pequenino Castro (Cagodo)', 'Avançado', 29],
  ['tchicundico-huila', 'Tchicundico Cassoma Tchitunda (Constantino)', 'Médio', 32],
  ['joao-samazanga-huila', 'João Baptista Ferraz Samazanga Juny (Jony)', 'Avançado', 34],
  ['geovany-huila', 'Geovany', 'Avançado', 35],
].map(([id, name, position, jerseyNumber]) => ({
  id: String(id), name: String(name), club: 'Desportivo da Huíla', teamId: 'desphuila', position: String(position),
  goals: 0, assists: 0, appearances: 0, jerseyNumber: Number(jerseyNumber), age: 0,
  nationality: 'Angola', height: 'A confirmar',
  attributes: { pace: 0, shooting: 0, passing: 0, dribbling: 0, defending: 0, physical: 0 }, careerHistory: [],
}));

// Plantel recebido da Académica do Lobito para a 2.ª jornada. A fonte não
// informa posições, por isso permanecem explicitamente por confirmar.
const LOBITO_SQUAD_2026_27: Player[] = [
  ['guilherme-lobito', 'Guilherme', 12],
  ['joel-lobito', 'Joel', 2],
  ['rosario-lobito', 'Rosário', 4],
  ['jorge-28-lobito', 'Jorge', 28],
  ...
  ['marcos-lobito', 'Marcos', 40],
  ['carlos-lobito', 'Carlos', 35],
  ['aurelio-lobito', 'Aurélio', 33],
  ['valerio-lobito', 'Valério', 29],
  ['jorge-enio-lobito', 'Jorge Enio', 27],
  ['wilson-david-lobito', 'Wilson David', 25],
  ['miguel-lobito', 'Miguel', 24],
  ['geraldo-lobito', 'Geraldo', 21],
  ['nanga-lobito', 'Nanga', 3],
].map(([id, name, jerseyNumber]) => ({
  id: String(id), name: String(name), club: 'Académica do Lobito', teamId: 'lobito', position: 'Posição por confirmar',
  goals: 0, assists: 0, appearances: 0, jerseyNumber: Number(jerseyNumber), age: 0,
  nationality: 'Angola', height: 'A confirmar',
  attributes: { pace: 0, shooting: 0, passing: 0, dribbling: 0, defending: 0, physical: 0 }, careerHistory: [],
}));
...
const BRAVOS_SQUAD_2026_27: Player[] = [
  ...
  ['tony-bravos', 'Tony', 'Médio', 25, 1, 0],
  ['saidi-bravos', 'Saidy', 'Guarda-redes', 12, 0, 0],
  ['bruno-bravos', 'Bruno', 'Defesa', 4, 0, 0],
...
const CABINDA_SQUAD_2026_27: Player[] = [
  ...
  ['bras-cabinda', 'José Brás', 'Avançado', 23, 1],
  ['cornelio-cabinda', 'Cornélio', 'Avançado', 15, 1],
  ['costa-cabinda', 'Costa', 'Médio', 7, 1],
  ['arsenio-cabinda', 'Arsénio', 'Guarda-redes', 31, 0],
  ['luis-cabinda', 'Luís', 'Defesa', 27, 0],
  ['jaime-cabinda', 'Jaime', 'Médio', 26, 0],
  ['mario-cabinda', 'Mário', 'Defesa', 6, 0],
].map(([id, name, position, jerseyNumber, appearances]) => ({
...
const OFFICIAL_FIFA_ID_COUNTS = OFFICIAL_SQUADS_2026_27.reduce((counts, squad) => {
  squad.players.forEach((record) => {
    if (record.fifaId) counts.set(record.fifaId, (counts.get(record.fifaId) ?? 0) + 1);
  });
  return counts;
  ...
  return squad.players.map((record) => {
    ...
    const fallback = (preferredId ? teamFallbacks.find((player) => player.id === preferredId) : undefined)
      ...
      ?? (canUseJerseyFallback && jerseyNumber > 0
        ...
        : undefined);
    const hasDuplicateFifaId = (OFFICIAL_FIFA_ID_COUNTS.get(record.fifaId) ?? 0) > 1;
...
export const TOP_SCORERS: PlayerStats[] = CURRENT_SEASON_SCORERS.map((player) => ({
  id: player.id,
  name: player.name,
  club: player.club,
...
export const newsMock: NewsArticle[] = [
  ...
  },
  {
    ...
    date: '14 set. 2026',
    isoDate: '2026-09-14',
    summary: 'A FAF delegou na ANCAF a organização, a regulamentação e a gestão do Campeonato Nacional de Futebol da 1.ª Divisão. O contrato, assinado a 24 de julho de 2026, vale para as épocas 2026/2027 e 2027/2028.',
    content: 'A Federação Angolana de Futebol (FAF) e a Associação Nacional de Clubes Angolanos de Futebol (ANCAF) assinaram em Luanda, a 24 de julho de 2026, o contrato que delega na ANCAF a competência de organizar, regulamentar e gerir o Campeonato Nacional de Futebol da 1.ª Divisão, designado no contrato por Liga Angolana de Futebol, bem como a Taça da Liga. O acordo fixa a competição entre 12 e 16 clubes, com subida direta dos dois primeiros classificados da 2.ª Divisão, um play-off em jogo único e descida dos dois últimos classificados, e sujeita os clubes ao licenciamento através do FIFA Connect. Define ainda as secções de disciplina e de arbitragem para a Liga, cabendo à ANCAF suportar os encargos das equipas de arbitragem e explorar comercialmente as competições que organiza. O contrato vale por duas épocas desportivas, de 24 de julho de 2026 a 24 de julho de 2028, com renovação automática, e prevê que a época 2026/2027 decorra com o auxílio técnico da FAF. O documento integral, com as assinaturas das duas partes, está disponível para consulta e descarga.',
    status: 'published',
    author: 'FAF e ANCAF',
    sourceName: 'ANCAF — Associação Nacional dos Clubes Angolanos de Futebol',
    sourceUrl: 'https://ancaf.co.ao',
    publishedAt: '2026-09-14T20:20:00+01:00',
    documentImages: [
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-1.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-2.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-3.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-4.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-5.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-6.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-7.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-8.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-9.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-10.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-11.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-12.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-13.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-14.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-15.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-16.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-17.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-18.jpg',
      '/comunicados/contrato-cedencia-gestao-girabola-2026/pagina-19.jpg',
    ],
    documentUrl: '/comunicados/contrato-cedencia-gestao-girabola-2026/contrato-cedencia-gestao-girabola-24-julho-2026.pdf',
  },
  {
...
export function getTeams(): Team[] {
  ...
  const extra = (ov.added ?? []).filter((a) => !base.some((b) => b.id === a.id)).map(applyTeamLogo);
  return [...base, ...extra];
}
...
export function getAllTeams(): Team[] {
  ...
  const promoted = PROMOTED_2026_27_TEAMS.filter((p) => !removed.has(p.id) && !teams.some((t) => t.id === p.id)).map(applyTeamLogo);
  return [
    ...teams,
    ...promoted,
    ...HISTORICAL_TEAMS.filter((h) => !removed.has(h.id) && !teams.some((t) => t.id === h.id)).map(applyTeamLogo),
  ];
}
...
/**
 * Clubes que disputam efetivamente uma época, derivados do respetivo
 * calendário. `getAllTeams()` inclui promovidos e clubes históricos de outras
 * edições (26 no total), pelo que não serve para filtrar estatísticas: o
 * seletor de clube tem de listar apenas os participantes da época escolhida.
 */
export function getSeasonTeams(seasonId: string): Team[] {
  const participating = new Set<string>();
  for (const match of getMatchesForSeason(seasonId)) {
    participating.add(match.homeTeamId);
    participating.add(match.awayTeamId);
  }

  const known = new Map(getAllTeams().map((team) => [team.id, team]));
  return [...participating]
    .map((teamId) => known.get(teamId) ?? ({
      id: teamId,
      name: getTeamFullName(teamId, teamId),
...
/**
 * As fichas da ANCAF publicam por vezes o mesmo árbitro com o nome abreviado
 * ("Gilberto Kativa") ou com uma grafia diferente ("Tchissingu"/"Tchissingui"),
 * o que partia a tabela de arbitragem em duas linhas para a mesma pessoa. Aqui
 * agrupamos apenas quando o primeiro e o último nome coincidem — critério
 * conservador que junta abreviaturas e gralhas sem fundir homónimos parciais
 * (ex.: "Bernardo Mário" e "Bernardo Hossi Nangolo" continuam separados) — e
 * elegemos como canónico o nome mais completo publicado. Os registos de origem
 * mantêm-se exatamente como foram recebidos.
 */
export function buildOfficialNameCanonicalizer(names: Iterable<string>): (name: string) => string {
  const groups = new Map<string, string[]>();
  for (const name of names) {
    const parts = name.trim().split(/\s+/).filter(Boolean);
    if (parts.length === 0) continue;
    const key = `${foldName(parts[0])}|${foldName(parts[parts.length - 1])}`;
  ...
  for (const bucket of groups.values()) {
    const best = bucket.reduce((longest, candidate) => (candidate.length > longest.length ? candidate : longest), bucket[0]);
    for (const name of bucket) canonical.set(name, best);
  }

  return (name: string) => canonical.get(name) ?? name;
}
...
// ── DETALHE DE JOGO ────────────────────────────────────────────────
// Na época ativa, estatísticas e ocorrências só são expostas quando constam
// das fichas recebidas. Os fallbacks históricos permanecem isolados e nunca
// são tratados como métricas oficiais da temporada 2026/2027.
...
export interface MatchEventDetail {
  ...
  ownGoal?: boolean;        // autogolo: conta para `team` e não entra nos marcadores
}
...
export interface MatchDetail {
  ...
  officialStatKeys: (keyof MatchTeamStats)[];
...
/** Escalação publicada no registo do jogo. */
function getRecordLineups(match: Match): { home: LineupPlayer[]; away: LineupPlayer[] } | undefined {
  const lineups = MATCH_RECORD_BY_ID.get(match.id)?.lineups;
  if (!lineups) return undefined;
  const withRating = (slot: MatchRecordLineupPlayer): LineupPlayer => ({ ...slot, rating: slot.rating ?? 0 });
  return { home: lineups.home.map(withRating), away: lineups.away.map(withRating) };
}

/** Eventos publicados no registo do jogo (golos, cartões e substituições) ou no arquivo de 2025/26. */
function getPublishedMatchEvents(match: Match): MatchEventDetail[] | undefined {
  return MATCH_RECORD_BY_ID.get(match.id)?.events ?? HISTORICAL_MATCH_EVENTS_2025_26[match.id];
}
...
const PUBLISHED_MATCH_STATS: Readonly<Record<string, PublishedMatchStats>> = Object.fromEntries(
  MATCH_RECORDS_2026_27.flatMap((record) => (record.stats ? [[record.id, record.stats]] : [])),
);

/**
 * Cartões de um clube conforme as fichas oficiais da época em curso. Usa o
...
/** Compara totais das fichas com os cartões já atribuídos nominalmente. */
export function getCurrentSeasonCardReconciliation() {
  ...
  const attributed = getCurrentSeasonDiscipline().reduce(
    (totals, player) => ({
      yellow: totals.yellow + player.yellowCards,
      red: totals.red + player.redCards,
    }),
    { yellow: 0, red: 0 },
  );
  ...
  return {
    yellowInSheets,
    redInSheets,
    yellowAttributed: attributed.yellow,
    redAttributed: attributed.red,
    yellowUnattributed: Math.max(0, yellowInSheets - attributed.yellow),
    redUnattributed: Math.max(0, redInSheets - attributed.red),
  };
}
...
export function getCurrentSeasonGoalReconciliation() {
  ...
  // Autogolos assinalados nos eventos dos registos de jogo (`ownGoal`).
  const ownGoals = SEASON_DERIVED_STATS_2026_27.ownGoals;
...
/**
 * Marcadores, cartões e autogolos da época em curso, calculados com as mesmas
 * escalações e eventos conciliados que a ficha de jogo mostra. Não corre em
 * tempo de execução: `npm run jogos -- sincronizar` grava o resultado em
 * src/data/jogos/2026-27/derivados.ts, e o build falha se esse ficheiro deixar
 * de corresponder aos registos.
 */
export function computeSeasonDerivedStats(): SeasonDerivedStats {
  const goals = new Map<string, { goals: number; teamId: string; name: string }>();
  const cards = new Map<string, { yellow: number; red: number }>();
  const participations = new Map<string, Set<string>>();
  let ownGoals = 0;
  const participate = (playerId: string | undefined, matchId: string) => {
    if (!playerId) return;
    const matchIds = participations.get(playerId) ?? new Set<string>();
    matchIds.add(matchId);
    participations.set(playerId, matchIds);
  };

  for (const match of getMatchesForSeason(UPCOMING_SEASON_ID)) {
    if (match.status !== 'finished') continue;
    const detail = getMatchDetail(match);
    for (const slot of [...detail.homeLineup, ...detail.awayLineup]) {
      if (slot.isStarter) participate(slot.playerId, match.id);
    }
    for (const event of detail.events) {
      if (event.type === 'goal' && (event.ownGoal || /autogolo/i.test(event.detail ?? ''))) {
        ownGoals += 1;
        continue;
      }
      if (!event.playerId) continue;
      participate(event.playerId, match.id);
      if (event.type === 'goal') {
        const teamId = event.team === 'home' ? match.homeTeamId : match.awayTeamId;
        const row = goals.get(event.playerId) ?? { goals: 0, teamId, name: event.player };
        row.goals += 1;
        goals.set(event.playerId, row);
      } else if (event.type === 'yellow' || event.type === 'red') {
        const row = cards.get(event.playerId) ?? { yellow: 0, red: 0 };
        row[event.type] += 1;
        cards.set(event.playerId, row);
      }
    }
  }

  const scorers = [...goals].map(([id, row]) => {
    // Atletas do registo curado usam o nome e a posição do plantel (os mesmos da
    // página do jogador e da disciplina); os restantes ficam com o nome publicado
    // na ficha. As entradas criadas a partir destes próprios totais não contam,
    // senão um valor antigo ficaria preso no ficheiro gerado.
    const known = currentPlayerBaseIds.has(id) ? getPlayerById(id) : undefined;
    return {
      id,
      name: known?.name ?? row.name,
      club: getTeamById(row.teamId)?.name ?? row.teamId,
      teamId: row.teamId,
      position: known?.position ?? 'Posição por confirmar',
      goals: row.goals,
      appearances: (participations.get(id)?.size ?? 0) + (APPEARANCES_WITHOUT_LINEUP_2026_27[id] ?? 0),
    };
  }).sort((a, b) => b.goals - a.goals || a.name.localeCompare(b.name, 'pt'));

  return {
    scorers,
    cards: Object.fromEntries([...cards].sort(([a], [b]) => a.localeCompare(b))),
    ownGoals,
  };
}
...
export function getMatchDetail(match: Match): MatchDetail {
  ...
  const publishedLineups = dbLineup ?? getRecordLineups(match);
  ...
  return {
    ...
    homeStats,
    awayStats,
    officialStatKeys: publishedStats?.keys ?? [],
    homeLineup,
    awayLineup,
    formationHome: '4-3-3',
    formationAway: '4-3-3',
...
// indisponível até ser recebido e publicado numa ficha oficial.
export function getMatchTempoUtil(match: Match): number | null {
  if (match.status !== 'finished' || match.usefulTimeMinutes === undefined) return null;
  return match.usefulTimeMinutes;
}
